...
def parse_args():
    ...
    parser.add_argument('-model', required=True, type=str, help='Targeting model.', 
                        choices=['metapath2vec-ESim','PTE','HIN2Vec','AspEm','HEER','R-GCN','HAN','MAGNN','HGT','TransE','DistMult','ComplEx','ConvE','R-GIN','CompGCN','DMPNN'])
    parser.add_argument('-attributed', required=True, type=str, help='Only DMPNN, CompGCN, R-GIN, R-GCN, HAN, MAGNN, and HGT support attributed training.',
                        choices=['True','False'])
    parser.add_argument('-supervised', required=True, type=str, help='Only DMPNN, CompGCN, R-GIN, R-GCN, HAN, MAGNN, and HGT support semi-supervised training.', 
                        choices=['True','False'])
...
def check(args):
    ...
    if args.attributed=='True':
        if args.model not in ['DMPNN', 'CompGCN', 'R-GIN', 'R-GCN','HAN', 'MAGNN', 'HGT']:
            print(f'{args.model} does not support attributed training!')
            print('Only DMPNN, CompGCN, R-GIN, R-GCN, HAN, MAGNN, and HGT support attributed training!')
            return False
    ...
    if args.supervised=='True':
        if args.model not in ['DMPNN', 'CompGCN', 'R-GIN', 'R-GCN','HAN', 'MAGNN', 'HGT']:
            ...
            print('Only DMPNN, CompGCN, R-GIN, R-GCN, HAN, MAGNN, and HGT support semi-supervised training!')
            return False
...
def main():
    ...
    elif args.model=='R-GIN': rgin_convert(args.dataset, args.attributed, args.supervised)
    elif args.model=='CompGCN': compgcn_convert(args.dataset, args.attributed, args.supervised)
    elif args.model=='DMPNN': dmpnn_convert(args.dataset, args.attributed, args.supervised)
